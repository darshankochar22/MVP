...
describe("Stock movement classification + valuation", () => {
  ...
  beforeAll(async () => {
    ...
    const parent = await stockGroupService.create({ company_id: companyId, name: "Parent Grp", parent_group_id: null });
    parentGroupId = parent.group.sg_id;
    const child = await stockGroupService.create({ company_id: companyId, name: "Child Grp", parent_group_id: parentGroupId });
    childGroupId = child.group.sg_id;

    const cat = await stockCategoryService.create({ company_id: companyId, name: "Widgets Cat", parent_id: null });
    categoryId = cat.category.sc_id;

    // Widget: opening 10 @ 100 = 1,000.
    const widget = await stockItemService.create({
      company_id: companyId, name: "Widget", group_id: childGroupId, category_id: categoryId,
      opening_quantity: 10, opening_rate: 100, opening_value: 1000,
    });
    ...
    const gadget = await stockItemService.create({
      company_id: companyId, name: "Gadget", group_id: parentGroupId,
    });
    ...
    await voucherService.create({
      ...
      stock_entries: [{ stock_item_id: widgetId, item_name: "Widget", quantity: 10, rate: 100, amount: 1000 }],
    });

    // 2. Sales 5 @ 200 = 1,000 (outward; amount is REVENUE, cost is 500).
    await voucherService.create({
      company_id: companyId, fy_id: fyId, voucher_type: "Sales",
      date: "2026-04-10", party_name: "Customer", is_inventory_voucher: 1, entries: [],
      stock_entries: [{ stock_item_id: widgetId, item_name: "Widget", quantity: 5, rate: 200, amount: 1000 }],
    });

    // 3. Credit Note 2 @ 200 = 400 (sales return → goods come BACK: inward).
    await voucherService.create({
      company_id: companyId, fy_id: fyId, voucher_type: "Credit Note",
      date: "2026-04-15", party_name: "Customer", is_inventory_voucher: 1, entries: [],
      stock_entries: [{ stock_item_id: widgetId, item_name: "Widget", quantity: 2, rate: 200, amount: 400 }],
    });
    ...
    await voucherService.create({
      ...
      date: "2026-04-20", is_accounting_voucher: 0, is_inventory_voucher: 1, entries: [],
      stock_entries: [
        { stock_item_id: widgetId, item_name: "Widget", quantity: 3, rate: 100, amount: 300, is_source: 1 },
        { stock_item_id: widgetId, item_name: "Widget", quantity: 3, rate: 100, amount: 300, is_source: 0 },
      ],
    });
    ...
    await voucherService.create({
      company_id: companyId, fy_id: fyId, voucher_type: "Purchase",
      date: "2026-04-25", party_name: "Supplier", is_inventory_voucher: 1, is_optional: 1, entries: [],
      stock_entries: [{ stock_item_id: widgetId, item_name: "Widget", quantity: 50, rate: 100, amount: 5000 }],
    });

    // 6. Production: Stock Journal destination-only 5 Gadgets @ 60.
    await voucherService.create({
      company_id: companyId, fy_id: fyId, voucher_type: "Stock Journal",
      date: "2026-04-22", is_accounting_voucher: 0, is_inventory_voucher: 1, entries: [],
      stock_entries: [
        { stock_item_id: gadgetId, item_name: "Gadget", quantity: 5, rate: 60, amount: 300, is_source: 0 },
      ],
    });
  });
  ...
  // FIFO closing value: lots 2@100(open rem) + 10@100 + 2@200 + 3@100 = 1,900.
  const WIDGET_FIFO_VALUE = 1900;
  ...
  it("valuation engine: Credit Note and Stock Journal move qty; optional excluded", async () => {
    ...
    expect(widget.closing_value).toBeCloseTo(WIDGET_FIFO_VALUE, 2);
  ...
  it("stockSummary: closing qty and value from the SAME engine (rate is sane)", async () => {
    ...
    expect(widget.closing_value).toBeCloseTo(WIDGET_FIFO_VALUE, 2);
    expect(widget.rate).toBeCloseTo(WIDGET_FIFO_VALUE / WIDGET_CLOSING_QTY, 2);
  ...
  it("stockGroupSummary: parent group rolls up child-group items", async () => {
    ...
    expect(childRow.value).toBeCloseTo(WIDGET_FIFO_VALUE, 2);
    // Parent = its own Gadget (5 @ 300) + child subtree (Widget).
    expect(parentRow.qty).toBe(WIDGET_CLOSING_QTY + 5);
    expect(parentRow.value).toBeCloseTo(WIDGET_FIFO_VALUE + 300, 2);
  });
  ...
  it("stockCategorySummary includes opening stock and excludes optional vouchers", async () => {
    ...
    expect(row.value).toBeCloseTo(WIDGET_FIFO_VALUE, 2);
  });

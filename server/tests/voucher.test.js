...
describe("Vouchers Service Tests", () => {
  ...
  it("should get ledger balances updating correctly", async () => {
    ...
    // PL balance should be 10000 Cr
    const plBal = await voucherService.getLedgerBalance(plLedgerId, companyId, fyId);
    expect(plBal.success).toBe(true);
    expect(plBal.rawBalance).toBe(10000); // production logic returns positive raw balance
    expect(plBal.balance).toBe("10000.00 Dr"); // production logic uses > 0.01 mapping to Dr label
  });

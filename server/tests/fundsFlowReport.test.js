...
describe("Fund Flow Report (Accounts)", () => {
  ...
  it("lists only non-current items + operations in sources/applications", async () => {
    const r = await fundsFlowReportService.fundsFlow(companyId, fyId, "2026-04-01", "2026-04-30");
    ...
    expect(src["Bank Loan (Increase)"]).toBe(500000);
    expect(app["Machinery (Increase)"]).toBe(300000);

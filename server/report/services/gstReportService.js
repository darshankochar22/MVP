 ...
 */
const {
  db, sql, INWARD_TYPES, OUTWARD_TYPES, sqlIn, baseVoucherFilter,
} = require('./reportHelpers');
const {
  vouchers, voucherStockEntries, voucherEntries, ledgers, groups, stockItems,
} = require('../../db/schema');

const round2 = (n) => Number((Number(n) || 0).toFixed(2));
const pct = (n) => `${Number(n) || 0}%`;
const partyLabel = (name, gstin) =>
  name ? (gstin ? `${name} (${gstin})` : name) : (gstin || '—');
...
const PORTAL_REPORTS = {
  ...
  gstr1_rejected:
    'Rejected-transaction status comes from the GST portal after upload.',
  gstr1_mismatched:
    'Mismatch status is determined by comparing books with portal-filed data.',
};
...
const UNSUPPORTED_REPORTS = {
  ...
  gst_audit_trail:
    'GST audit trail of edits is available via the dedicated Audit Trail report.',
};
...
// Per-invoice tax totals for a document filter (Shape A source rows).
...
async function totals(base, docCond) {
  const r = await db.all(sql`
    SELECT COALESCE(SUM(vse.amount), 0) AS taxable_value,
           COALESCE(SUM(vse.cgst_amount), 0) AS cgst,
           COALESCE(SUM(vse.sgst_amount), 0) AS sgst,
           COALESCE(SUM(vse.igst_amount), 0) AS igst
    FROM ${vouchers} v
...
// Rate-wise breakup (Shape A category rows) for a document filter.
async function byRate(base, docCond) {
  const rows = await db.all(sql`
    SELECT COALESCE(vse.gst_rate, 0) AS gst_rate,
           COALESCE(SUM(vse.amount), 0) AS taxable_value,
           COALESCE(SUM(vse.cgst_amount), 0) AS cgst,
           COALESCE(SUM(vse.sgst_amount), 0) AS sgst,
           COALESCE(SUM(vse.igst_amount), 0) AS igst
    ...
    INNER JOIN ${voucherStockEntries} vse ON vse.voucher_id = v.voucher_id
    WHERE ${sql.join([...base, sql`v.is_invoice = 1`, docCond], sql` AND `)}
    GROUP BY COALESCE(vse.gst_rate, 0)
...
async function getGstReport(company_id, fy_id, arg = {}) {
  try {
    ...
    if (PORTAL_REPORTS[reportType]) {
      return { success: true, rows: [], message: PORTAL_REPORTS[reportType], portal_required: true };
    }
    if (UNSUPPORTED_REPORTS[reportType]) {
      return { success: true, rows: [], message: UNSUPPORTED_REPORTS[reportType], not_available: true };
    }
    ...
    switch (reportType) {
      ...
      case 'gstr1_b2b': {
        const rows = await invoiceRows(base, sql`(${SALES}) AND l.gstin IS NOT NULL AND l.gstin != ''`);
        return { success: true, rows: rows.map((r) => rowA(r.voucher_number, partyLabel(r.party_name, r.party_gstin), r, 'B2B')) };
      }
      case 'gstr1_b2c_large': {
        ...
        return { success: true, rows: (large.length ? large : rows).map((r) => rowA(r.voucher_number, partyLabel(r.party_name, ''), r, 'B2C (Large)')) };
      }
      case 'gstr1_b2c_small': {
        ...
        for (const r of rows) {
          ...
          byPos[pos].cgst += Number(r.cgst); byPos[pos].sgst += Number(r.sgst); byPos[pos].igst += Number(r.igst);
        }
        return { success: true, rows: Object.entries(byPos).map(([pos, t]) => rowA(`POS: ${pos}`, '', t, 'B2C (Small)')) };
      }
      case 'gstr1_cd_notes':
      case 'gstr1_cdnr': {
        const rows = await invoiceRows(base, NOTES);
        return { success: true, rows: rows.map((r) => rowA(`${r.voucher_type} ${r.voucher_number}`, partyLabel(r.party_name, r.party_gstin), r, r.voucher_type)) };
      }
      case 'gstr1_nil': {
        const rows = await invoiceRows(base, SALES);
        const nil = rows.filter((r) => round2(r.cgst) + round2(r.sgst) + round2(r.igst) === 0);
        return { success: true, rows: nil.map((r) => rowA(r.voucher_number, partyLabel(r.party_name, r.party_gstin), r, 'Nil/Exempt')) };
      }
      case 'gstr1_export': {
        const rows = await invoiceRows(base, sql`(${SALES}) AND l.country IS NOT NULL AND l.country != '' AND LOWER(l.country) != 'india'`);
        if (rows.length === 0) {
          return { success: true, rows: [], message: 'No export invoices found. Exports are identified by a party whose country is set to something other than India.' };
        }
        return { success: true, rows: rows.map((r) => rowA(r.voucher_number, partyLabel(r.party_name, r.party_country), r, 'Export')) };
      }
      ...
      case 'gstr1_uncertain': {
        ...
        return { success: true, rows: bad.map((r) => {
          const reg = r.party_reg_type && r.party_reg_type !== 'Unregistered';
          const gstin = (r.party_gstin || '').trim();
          const reason = reg && !gstin ? 'Missing GSTIN' : gstin && gstin.length !== 15 ? 'Invalid GSTIN' : 'Missing Place of Supply';
      ...
      case 'gstr1_included': {
        ...
        return { success: true, rows: ok.map((r) => rowA(r.voucher_number, partyLabel(r.party_name, r.party_gstin), r, 'Included')) };
      }
      case 'gstr1_not_relevant': {
        ...
        return { success: true, rows: rows.map((r) => ({
          ...
          party_gstin: r.party_name || '', taxable_value: 0, igst: 0, cgst: 0, sgst: 0,
          status: 'Not relevant',
        })) };
      }
      ...
      case 'hsn_summary': {
        const rows = await db.all(sql`
          SELECT COALESCE(NULLIF(vse.hsn_code, ''), si.hsn_code, 'N/A') AS hsn_code,
                 COALESCE(si.hsn_sac_description, vse.item_name, '') AS description,
          ...
          GROUP BY COALESCE(NULLIF(vse.hsn_code, ''), si.hsn_code, 'N/A'), COALESCE(vse.gst_rate, si.gst_rate, 0)
          ORDER BY hsn_code ASC`);
        return { success: true, rows: rows.map((r) => rowA(`${r.hsn_code}${r.description ? ' — ' + r.description : ''}`, pct(r.gst_rate), r, '')) };
      }
      ...
      case 'gstr3b_exempt': {
        const out = await totals(base, sql`(${SALES}) AND COALESCE(vse.gst_rate, 0) = 0`);
        const inn = await totals(base, sql`(${PURCHASE}) AND COALESCE(vse.gst_rate, 0) = 0`);
      ...
      case 'annual_computation': {
        ...
        return { success: true, rows: [
          ...
          { section_invoice: 'Net GST payable', party_gstin: '', taxable_value: 0,
      ...
      case 'missing_gstin': {
        const rows = await invoiceRows(base, sql`(${SALES}) AND (l.gstin IS NULL OR l.gstin = '') AND l.registration_type IS NOT NULL AND l.registration_type != 'Unregistered'`);
        return { success: true, rows: rows.map((r) => rowA(r.voucher_number, partyLabel(r.party_name, ''), r, 'Missing GSTIN')) };
      }
      case 'gstin_validation': {
        const rows = await invoiceRows(base, sql`(${SALES}) AND l.gstin IS NOT NULL AND l.gstin != '' AND length(l.gstin) != 15`);
        return { success: true, rows: rows.map((r) => rowA(r.voucher_number, partyLabel(r.party_name, r.party_gstin), r, 'Invalid GSTIN')) };
      }
      case 'place_of_supply': {
        const rows = await invoiceRows(base, sql`(${SALES}) AND COALESCE(v.place_of_supply, '') = ''`);
        return { success: true, rows: rows.map((r) => rowA(r.voucher_number, partyLabel(r.party_name, r.party_gstin), r, 'Missing Place of Supply')) };
      }
      case 'missing_hsn': {
        const rows = await db.all(sql`
          SELECT v.voucher_number, v.party_name, vse.item_name,
                 COALESCE(vse.amount, 0) AS taxable_value,
                 COALESCE(vse.cgst_amount, 0) AS cgst, COALESCE(vse.sgst_amount, 0) AS sgst,
          ...
          ORDER BY v.date ASC`);
        return { success: true, rows: rows.map((r) => rowA(`${r.voucher_number} — ${r.item_name || 'Item'}`, r.party_name || '', r, 'Missing HSN/SAC')) };
      }
      case 'gst_exception': {
        const noGstin = await invoiceRows(base, sql`(${SALES}) AND (l.gstin IS NULL OR l.gstin = '') AND l.registration_type IS NOT NULL AND l.registration_type != 'Unregistered'`);
        const badGstin = await invoiceRows(base, sql`(${SALES}) AND l.gstin IS NOT NULL AND l.gstin != '' AND length(l.gstin) != 15`);
        const noPos = await invoiceRows(base, sql`(${SALES}) AND COALESCE(v.place_of_supply, '') = ''`);
        const rows = [
          ...noGstin.map((r) => rowA(r.voucher_number, partyLabel(r.party_name, ''), r, 'Missing GSTIN')),
          ...badGstin.map((r) => rowA(r.voucher_number, partyLabel(r.party_name, r.party_gstin), r, 'Invalid GSTIN')),
          ...noPos.map((r) => rowA(r.voucher_number, partyLabel(r.party_name, r.party_gstin), r, 'Missing Place of Supply')),
        ];
        if (rows.length === 0) return { success: true, rows: [], message: 'No GST exceptions — all invoices have a valid GSTIN and place of supply.' };
        return { success: true, rows };
      }
      ...
      case 'party_wise_purchase': {
        ...
        const rows = await db.all(sql`
          SELECT ${group} AS grp,
                 ...
                 COALESCE(SUM(vse.amount), 0) AS taxable,
                 COALESCE(SUM(vse.cgst_amount + vse.sgst_amount + vse.igst_amount), 0) AS tax
          FROM ${voucherStockEntries} vse
          INNER JOIN ${vouchers} v ON v.voucher_id = vse.voucher_id
          LEFT JOIN ${ledgers} l ON l.ledger_id = v.party_ledger_id
          WHERE ${sql.join([...base, sql`v.is_invoice = 1`, docCond], sql` AND `)}
        ...
        return { success: true, rows: rows.map((r) => ({
          party_item: reportType.startsWith('rate_wise') ? `Taxable @ ${pct(r.grp)}` : (r.grp || 'Unspecified'),
      ...
      case 'liability_register': {
        const rows = await db.all(sql`
          SELECT strftime('%Y-%m', v.date) AS ym,
                 SUM(CASE WHEN v.voucher_type = 'Sales' THEN COALESCE(vse.cgst_amount,0)+COALESCE(vse.sgst_amount,0)+COALESCE(vse.igst_amount,0) ELSE 0 END) AS output_tax,
                 SUM(CASE WHEN v.voucher_type = 'Purchase' THEN COALESCE(vse.cgst_amount,0)+COALESCE(vse.sgst_amount,0)+COALESCE(vse.igst_amount,0) ELSE 0 END) AS itc
          FROM ${vouchers} v
          INNER JOIN ${voucherStockEntries} vse ON vse.voucher_id = v.voucher_id
        ...
        for (const r of rows) {
          ...
          out.push({ date: `${r.ym}-01`, particulars: 'Output tax liability', type_section: r.ym, ref_vch_no: '', amount: round2(r.output_tax), status: '' });
          out.push({ date: `${r.ym}-01`, particulars: 'Less: input tax credit', type_section: r.ym, ref_vch_no: '', amount: round2(-r.itc), status: '' });
          out.push({ date: `${r.ym}-01`, particulars: 'Net GST payable', type_section: r.ym, ref_vch_no: '', amount: net, status: net >= 0 ? 'Payable' : 'Credit c/f' });
        }
      ...
      case 'payment_register': {
        const rows = await db.all(sql`
          ...
          ORDER BY v.date ASC`);
        return { success: true, rows: rows.map((r) => ({
          date: r.date, particulars: r.ledger_name, type_section: r.voucher_type,
          ref_vch_no: r.voucher_number, amount: round2(r.amount),
      ...
      case 'output_tax_ledger': {
        ...
        return { success: true, rows: rows.map((r) => {
          ...
          return {
            ...
            vch_type: r.voucher_type, vch_no: r.voucher_number,
      ...
      case 'menu':
        return { success: true, rows: [], message: 'Use the GST Reports menu to open a specific report.' };

      default:
        return { success: true, rows: [], message: `GST report "${reportType}" is not available from books.` };
    }
  } catch (err) {

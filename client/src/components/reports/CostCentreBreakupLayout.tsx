...
interface CostCentreMeta { cc_id: number; name: string }
...
export default function CostCentreBreakupLayout() {
  ...
  React.useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    (window as any).api.report.run("cost_centre_ledger", { company_id: cid, fy_id: fyid, cost_centre_id: costCentreId })
      .then((res: any) => {
        if (res?.success) {
  ...
  const filteredCentres = costCentres.filter(c => c.name.toLowerCase().includes(search.toLowerCase()));
  ...
  React.useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowDown") { e.preventDefault(); setPickerFocus(p => Math.min(filteredCentres.length - 1, p + 1)); }
      else if (e.key === "ArrowUp") { e.preventDefault(); setPickerFocus(p => Math.max(0, p - 1)); }
      else if (e.key === "Enter") {
        e.preventDefault();
        const c = filteredCentres[pickerFocus];
        if (c) { setCostCentreId(c.cc_id); setCostCentreName(c.name); }
  ...
  React.useEffect(() => {
    if (!costCentreId) return;
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "ArrowDown") { e.preventDefault(); setFocusedIndex(p => Math.min(groupedRows.length - 1, p + 1)); }
      else if (e.key === "ArrowUp") { e.preventDefault(); setFocusedIndex(p => Math.max(0, p - 1)); }
      else if (e.key === "Enter") {
        e.preventDefault();
        const r = groupedRows[focusedIndex];
        if (r) {
          navigate(`/reports/accounts/cost-centre-ledger?cost_centre_id=${costCentreId}&cost_centre_name=${encodeURIComponent(costCentreName)}&ledger_name=${encodeURIComponent(r.ledger_name)}`);
        }
      } else if (e.key === "Escape" || e.key === "Backspace") {
        e.preventDefault();
        setCostCentreId(null); setCostCentreName(""); setDetailRows([]); setFocusedIndex(0);
      }
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
        ...
        <table className="w-full border-collapse text-[11px] font-mono">
          ...
          <tbody>
            {groupedRows.length === 0 ? (
              <tr>
                <td colSpan={3} className="px-4 py-8 text-center text-zinc-400 italic">No allocations found.</td>
              </tr>
            ) : (

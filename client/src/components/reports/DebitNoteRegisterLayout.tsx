...
export default function DebitNoteRegisterLayout() {
  ...
  React.useEffect(() => {
    ...
    (window as any).api.report
      .debitNoteRegister(companyId, fyId)
      .then((res: any) => {
        if (res.success) {
          setMonthRows(res.rows || []);
          setFocusedMonthIndex(0);
        } else {
          setError(res.error || "Failed to load Debit Note Register");
        }
      })
  ...
  const loadVouchersForMonth = React.useCallback(
    (monthRow: MonthRow) => {
      ...
      (window as any).api.report
        .debitNoteRegisterVouchers(companyId, fyId, range.from, range.to)
        .then((res: any) => {
          if (res.success) {
            setVoucherRows(res.rows || []);
          } else {
            setError(res.error || "Failed to load Debit Note vouchers");
          }
        })
        .catch((err: any) => setError(err.message))
            ...
            <thead className="sticky top-0 bg-white border-b border-black z-10 text-black">
              ...
              <tr className="bg-white">
                <th colSpan={6} className="px-4 py-0.5 text-right font-normal italic text-black/60 border-b border-black/10">
                  List of All Debit Note Vouchers — {selectedCompany?.name} — {selectedMonth.month}
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                voucherRows.map((row, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-1.5">{row.voucher_type || "Debit Note"}</td>

...
export default function ReceiptRegisterLayout() {
  ...
  React.useEffect(() => {
    ...
    (window as any).api.report
      .receiptRegister(companyId, fyId)
      .then((res: any) => {
        if (res.success) {
          setMonthRows(res.rows || []);
          setFocusedMonthIndex(0);
        } else {
          setError(res.error || "Failed to load Receipt Register");
        }
      })
  ...
  const loadVouchersForMonth = React.useCallback(
    (monthRow: MonthRow) => {
      ...
      (window as any).api.report
        .receiptRegisterVouchers(companyId, fyId, range.from, range.to)
        .then((res: any) => {
          if (res.success) {
            setVoucherRows(res.rows || []);
          } else {
            setError(res.error || "Failed to load Receipt vouchers");
          }
        })
        .catch((err: any) => setError(err.message))
            ...
            <thead className="sticky top-0 bg-[#e5eff5] border-b border-zinc-300 z-10 text-zinc-700">
              ...
              <tr className="bg-[#e5eff5]">
                <th colSpan={6} className="px-4 py-0.5 text-right font-normal italic text-zinc-500 border-b border-zinc-200">
                  List of All Receipt Vouchers — {selectedCompany?.name} — {selectedMonth.month}
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                voucherRows.map((row, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-1.5">{row.voucher_type || "Receipt"}</td>

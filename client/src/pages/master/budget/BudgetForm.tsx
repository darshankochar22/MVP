...
export default function BudgetForm({
  ...
  const ccOptions = [{ value: "", label: "Not Applicable" }, ...costCentres.map((c) => ({ value: c.cc_id!, label: c.name }))];
  ...
  const yesNo = (on: boolean) => (
    <span className={`text-[12px] font-bold font-mono ${on ? "text-zinc-950" : "text-zinc-400"}`}>{on ? "Yes" : "No"}</span>
  );
            ...
            <div className="px-4 pb-5">
              <div className="border border-zinc-300">
                ...
                <div className="grid grid-cols-[1fr_1fr_1fr_1fr_1fr] text-center">
                  ...
                  <button onClick={() => setSubScreen("costCentre")} className="py-1.5 hover:bg-zinc-50">
...
function NameCostTypeAmountScreen({
  ...
  const title = kind === "group" ? `Group Budgets Under '${budgetName}'` : `Ledger Budgets Under '${budgetName}'`;

  const update = (i: number, patch: any) => onChange(rows.map((r, idx) => (idx === i ? { ...r, ...patch } : r)));
  const addRow = () => onChange([...rows, { [idKey]: 0, cost_centre_id: null, type_of_budget: "On Closing Balance", amount: 0 }]);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col font-mono text-[12px]">
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-4xl mx-auto border border-zinc-300">
          ...
          {rows.length === 0 && (
            <div className="py-6 text-center text-zinc-400 italic font-sans text-xs">No allocations yet — add a row below.</div>
          )}
          {rows.map((r, i) => (
            <div key={i} className="grid grid-cols-[3fr_2fr_2fr_1.5fr_40px] border-b border-zinc-100 items-center">
              <div className="border-r border-zinc-100 px-1">
                <Select
                  className="border-0 h-7"
                  value={r[idKey] || ""}
                  onChange={(e) => update(i, { [idKey]: e.target.value ? Number(e.target.value) : 0 })}
              ...
              <div className="border-r border-zinc-100 px-1">
                <Select
                  ...
                  onChange={(e) => update(i, { cost_centre_id: e.target.value ? Number(e.target.value) : null })}
                  options={ccOptions}
                />
              </div>
              <div className="border-r border-zinc-100 px-1">
          ...
          <div className="grid grid-cols-[3fr_2fr_2fr_1.5fr_40px] border-t border-zinc-300 bg-zinc-50 font-bold">
            ...
            <div className="py-1.5 px-2 text-right text-zinc-900">
              {rows.reduce((s, r) => s + (Number(r.amount) || 0), 0).toLocaleString("en-IN", { minimumFractionDigits: 2 })}
            </div>
            <div />
          </div>
        </div>
...
function CostCentreScreen({
  ...
  const addRow = () => onChange([...rows, { cost_centre_id: 0, expenses: 0, income: 0, closing_balance: 0 }]);
  const removeRow = (i: number) => onChange(rows.filter((_, idx) => idx !== i));
    ...
    <div className="fixed inset-0 z-50 bg-white flex flex-col font-mono text-[12px]">
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-4xl mx-auto border border-zinc-300">
          ...
          {rows.map((r, i) => (
            <div key={i} className="grid grid-cols-[3fr_2fr_2fr_2fr_40px] border-b border-zinc-100 items-center">
              <div className="border-r border-zinc-100 px-1">
                <Select
                  className="border-0 h-7"
                  value={r.cost_centre_id || ""}
                  onChange={(e) => update(i, { cost_centre_id: e.target.value ? Number(e.target.value) : 0 })}
                  options={ccOptions}
                  placeholder="Select…"
                />
              </div>
              <div className="border-r border-zinc-100 px-1">
                <input
                  ...
                  type="number"
                  value={r.expenses ?? 0}
                  onChange={(e) => update(i, { expenses: e.target.value === "" ? 0 : Number(e.target.value) })}
                />
              </div>
              <div className="border-r border-zinc-100 px-1">
                <input
                  ...
                  type="number"
                  value={r.income ?? 0}
                  onChange={(e) => update(i, { income: e.target.value === "" ? 0 : Number(e.target.value) })}
                />
              </div>
              <div className="border-r border-zinc-100 px-1">
                <input
                  ...
                  onChange={(e) => update(i, { closing_balance: e.target.value === "" ? 0 : Number(e.target.value) })}
                />
              </div>
              <button onClick={() => removeRow(i)} className="text-zinc-400 hover:text-red-500 text-sm font-bold">
                &times;

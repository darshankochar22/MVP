      ...
      <div className="flex-1 flex overflow-hidden min-h-0 bg-white">
        <div className="flex-1 flex flex-col min-w-0 bg-white h-full border-r border-zinc-100">
          ...
          <div className="flex-1 overflow-y-auto min-h-0 bg-white">
            {loading ? (
              <div className="flex items-center justify-center h-48 text-xs text-zinc-400 font-sans italic">Loading budgets...</div>
            ) : filtered.length === 0 ? (
              <div className="flex items-center justify-center h-48 text-xs text-zinc-400 font-sans italic">No budgets found.</div>
            ) : (
              filtered.map((b) => (
                <div
                  key={b.budget_id}
                  ...
                  className={`grid grid-cols-[4fr_3fr_3fr] border-b border-zinc-100 cursor-pointer text-[12px] ${
                    selectedId === b.budget_id ? "bg-zinc-100 font-bold text-black" : "text-zinc-700 hover:bg-zinc-50"
                  }`}
                >
                  <div className="py-1.5 px-3 border-r border-zinc-100 truncate">{b.name}</div>
                  <div className="py-1.5 px-3 border-r border-zinc-100 truncate text-zinc-500">{parentName(b)}</div>

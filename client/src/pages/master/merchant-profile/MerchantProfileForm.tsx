...
export default function MerchantProfileForm({
    ...
    try {
      ...
      const res =
        mode === "create"
          ...
          : await window.api.merchantProfile.update({ ...payload, merchant_profile_id: initial?.merchant_profile_id });
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape") { e.preventDefault(); (onBack ?? onCancel)(); }
      if ((e.altKey || e.ctrlKey) && e.key.toLowerCase() === "a") { e.preventDefault(); handleSubmit(); }
      if (mode === "alter" && e.altKey && e.key.toLowerCase() === "d") { e.preventDefault(); onDelete?.(); }
    };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none">
      <PageTitleBar title={mode === "create" ? "Merchant Profile Creation" : "Merchant Profile Alteration"} />
      ...
      <div className="flex-1 flex min-h-0">
        <div className="flex-1 flex flex-col min-w-0 bg-white overflow-y-auto">
          <div className="p-6 max-w-[680px]">
            <div className="text-center text-sm font-bold text-zinc-800 mb-4">Merchant Profile Details</div>

            <FormRow label="Name" labelWidth={LABEL_W} required className="flex items-center min-h-[26px]">
              <input autoFocus className={inputCls} value={name} onChange={(e) => setName(e.target.value)} />
            </FormRow>

            <FormRow label="Payment Method (Payment Gateway/UPI)" labelWidth={LABEL_W} className="flex items-center min-h-[26px]">

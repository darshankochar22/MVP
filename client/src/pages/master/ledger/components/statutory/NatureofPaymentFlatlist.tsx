...
interface NatureOfPaymentFlatListProps {
  items: TDSNatureOfPaymentType[];
  /** Currently selected value — compared against name, or against the
   *  pinned literal ("Any" / "Undefined") for virtual rows. */
  selectedValue?: string | null;
  onSelect?: (value: string, record?: TDSNatureOfPaymentType) => void;
  onCreate?: () => void;
  onClose?: () => void;
  title?: string;
  showHeader?: boolean;
...
export default function NatureOfPaymentFlatList({
  items,
  selectedValue,
  onSelect,
  onCreate,
  ...
  showHeader = true,
  loading = false,
}: NatureOfPaymentFlatListProps) {
  // name is optional on the real type — filter out any rows missing it
  // (shouldn't happen for real DB rows, but keeps this safe) and sort.
  const sortedItems = items
    .filter((item): item is TDSNatureOfPaymentType & { name: string } =>
      Boolean(item.name)
    )
  ...
  const Header = showHeader && (
    <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-200 bg-zinc-50 select-none">
      <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">
        {title || "List of Nature of Payments"}
      </span>
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {loading && (
          <div className="text-sm text-zinc-400 px-3 py-2">Loading…</div>
        )}

        {!loading && sortedItems.length === 0 && (
          <div className="text-sm text-zinc-400 px-3 py-2">
            No saved Nature of Payments
          </div>
        )}

...
export default function PricelistscAlter() {
      ...
      try {
        ...
        if (id && window.api?.priceList) {
          ...
          if (result?.success && result?.data) {
            ...
            setSelectedCategory(record.stock_category ?? "All Items");
  ...
  }, [companyId, id, selectedCategory, selectedLevel, applicableFrom, lines, navigate]);
  ...
  }, [handleSubmit, navigate, showCategoryList, showLevelList, activeItemDropdown]);
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="border-b border-zinc-200 bg-white px-6 py-3 shrink-0">
            <div className="grid grid-cols-[180px_1fr] gap-y-1.5 gap-x-4 max-w-lg text-xs font-mono">
              ...
              {/* Stock Category Name */}
              <span className="text-zinc-500 flex items-center">Stock Category Name</span>
                ...
                {showCategoryList && (
                  <div className="absolute left-0 top-full mt-1 z-50 bg-white border border-zinc-300 rounded shadow-lg w-64 max-h-56 overflow-y-auto">
                    <div className="px-3 py-1.5 text-[10px] font-bold text-zinc-400 uppercase tracking-wider border-b border-zinc-100 flex justify-between">
                      <span>List of Stock Categories</span>
                    </div>
                    {[{ sc_id: 0, name: "All Items" }, ...stockCategories].map((sc) => (
                      <div
                        key={sc.sc_id}
                        className={`px-3 py-1.5 text-xs cursor-pointer hover:bg-amber-100 ${selectedCategory === sc.name ? "bg-amber-200 font-bold" : ""}`}
                        onClick={() => { setSelectedCategory(sc.name); setShowCategoryList(false); }}
                      >
                        {sc.name === "All Items" && <span className="text-zinc-400 mr-1">◆</span>}
                        {sc.name}
                      </div>
                    ))}
                  </div>
          ...
          <div className="border-b border-zinc-200 bg-zinc-50 px-6 py-1.5 flex items-center gap-8 text-[11px] font-mono shrink-0">
            <span>
              <span className="text-zinc-400">Under Category</span>
              <span className="mx-2 text-zinc-300">:</span>
              <span className="text-zinc-400">◆</span>
              <span className="font-bold text-zinc-800 ml-1">{selectedCategory}</span>
            </span>
            <span>

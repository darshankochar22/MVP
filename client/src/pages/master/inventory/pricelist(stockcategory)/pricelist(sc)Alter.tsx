...
interface StockCategory { sc_id: number; name: string; }
...
const emptyLine = (): PriceListLine => ({
  particulars: "", item_id: null, qty_from: "", qty_less_than: "", rate: "", disc_percent: "",
});
...
export default function PricelistscAlter() {
  ...
  const [applicableFrom, setApplicableFrom] = useState<string>(new Date().toISOString().slice(0, 10));
  ...
  const mergedLevels = useMemo(() => {
    ...
    for (const l of [...priceLevels, ...DEFAULT_LEVELS]) {
      ...
      if (l.trim() && !seen.has(key)) { seen.add(key); out.push(l.trim()); }
    }
    return out;
  }, [priceLevels]);

  // Only category-based price lists are altered here (stock_category present).
  const categoryRecords = useMemo(
    () => records.filter((r) => r.stock_category != null && String(r.stock_category).trim() !== ""),
    [records]
  );
    ...
    const load = async () => {
      try {
        ...
        if (window.api?.priceLevels) {
          ...
          if (pl?.success && pl?.data) setPriceLevels((pl.data as string[]).filter((n) => n.trim() !== ""));
        }
  ...
  const openRecord = (rec: PriceListRecord) => {
    ...
    setSelectedCategory(rec.stock_category ?? "All Items");
    setSelectedLevel(rec.price_level ?? "");
    setApplicableFrom(rec.applicable_from ? rec.applicable_from.slice(0, 10) : new Date().toISOString().slice(0, 10));
  ...
  // ── Line helpers
  const setLineField = (index: number, field: keyof PriceListLine, value: string | number | null) => {
    setLines((prev) => {
      const next = [...prev];
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!editingId)      { setError("No price list selected."); return; }
    ...
    try {
      if (window.api?.priceList) {
        const result = await window.api.priceList.update({
          ...
          stock_category: selectedCategory,
      ...
      setTimeout(() => { setSuccess(null); backToSelect(); }, 1000);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to update price list.");
    } finally {
      setLoading(false);
    }
  }, [companyId, editingId, selectedCategory, selectedLevel, applicableFrom, lines]);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        if (showCategoryList || showLevelList || activeItemDropdown !== null) {
          setShowCategoryList(false); setShowLevelList(false); setActiveItemDropdown(null); return;
        }
        e.preventDefault();
        if (step === "edit") { backToSelect(); return; }
  ...
  }, [handleSubmit, handleDelete, navigate, showCategoryList, showLevelList, activeItemDropdown, step]);

  // ── Row keyboard nav
  const handleParticularKeyDown = (e: React.KeyboardEvent, i: number) => {
    if (e.key === "Enter") {
      e.preventDefault();
      if (lines[i].particulars.trim() !== "") { setActiveItemDropdown(null); qtyFromRefs.current[i]?.focus(); }
    }
    if (e.key === "Backspace" && lines[i].particulars === "" && lines.length > 1) { e.preventDefault(); removeLine(i); }
  };

  const handleCellKeyDown = (e: React.KeyboardEvent, rowIndex: number, nextRef: React.MutableRefObject<(HTMLInputElement | null)[]>) => {
    if (e.key === "Enter" || e.key === "Tab") { e.preventDefault(); nextRef.current[rowIndex]?.focus(); }
  };

  const handleDiscKeyDown = (e: React.KeyboardEvent, rowIndex: number) => {
    if (e.key === "Enter" || e.key === "Tab") { e.preventDefault(); particularRefs.current[rowIndex + 1]?.focus(); }
  };
  ...
  if (step === "select") {
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-950">
        <PageTitleBar title="Price List (Stock Category) — Alter" subtitle={selectedCompany?.name} />
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {fetching ? (
            ...
            <div className="p-6 text-sm text-zinc-400 font-sans">
              No saved category price lists found.{" "}
              <button onClick={() => navigate("/master/create/price-lists-sc")} className="text-zinc-900 underline font-bold">Create one</button>
            </div>
          ) : (
            <table className="w-full text-[11px] font-mono border-collapse">
              <thead className="sticky top-0 z-10">
                <tr className="bg-zinc-100 border-b border-zinc-300 text-left">
                  ...
                  <th className="px-4 py-2 font-bold text-zinc-600">Under Category</th>
              ...
              <tbody>
                {categoryRecords.map((rec, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-zinc-700">◆ {rec.stock_category || "All Items"}</td>
                    <td className="px-4 py-2 text-zinc-700">{formatDateDisplay(rec.applicable_from)}</td>
                    <td className="px-4 py-2 text-right text-zinc-500">{(rec.lines ?? []).length}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="flex-1 flex min-h-0 relative">
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="border-b border-zinc-200 px-6 py-3 shrink-0 font-mono text-[11px] space-y-2">
            ...
            <div className="flex items-center gap-2">
              <span className="text-zinc-500 w-28">Under Category</span>
              ...
              <button
                ...
                onClick={() => { setShowCategoryList((p) => !p); setShowLevelList(false); }}
              >
            ...
            <div className="flex items-center gap-10">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={() => { setShowLevelList((p) => !p); setShowCategoryList(false); }}
                >
              ...
              <div className="flex items-center gap-2">
                ...
                />
                {applicableFrom && <span className="text-zinc-400">{formatDateDisplay(applicableFrom)}</span>}
                <span className="ml-6 text-zinc-400">{filledCount} {filledCount === 1 ? "item" : "items"}</span>
              </div>
            </div>
          </div>
                  ...
                  return (
                    <tr key={i} className={`border-b border-zinc-100 group ${isLastEmpty ? "bg-zinc-50" : "hover:bg-zinc-50"} ${activeItemDropdown === i ? "bg-zinc-100" : ""}`}>
                      ...
                      <td className="px-2 py-1 align-middle w-24">
                        <input ref={(el) => { qtyFromRefs.current[i] = el; }} className={cellCls + " text-right"} value={line.qty_from} placeholder="0"
                          onChange={(e) => setLineField(i, "qty_from", e.target.value)} onKeyDown={(e) => handleCellKeyDown(e, i, qtyUpToRefs)} />
                      </td>
                      <td className="px-2 py-1 align-middle w-24">
                        <input ref={(el) => { qtyUpToRefs.current[i] = el; }} className={cellCls + " text-right"} value={line.qty_less_than} placeholder="0"
                          onChange={(e) => setLineField(i, "qty_less_than", e.target.value)} onKeyDown={(e) => handleCellKeyDown(e, i, rateRefs)} />
                      </td>
                      <td className="px-2 py-1 align-middle">
                        <input ref={(el) => { rateRefs.current[i] = el; }} className={cellCls + " text-right"} value={line.rate} placeholder="0.00"
                          onChange={(e) => setLineField(i, "rate", e.target.value)} onKeyDown={(e) => handleCellKeyDown(e, i, discRefs)} />
                      </td>
                      <td className="px-2 py-1 align-middle">
                        <input ref={(el) => { discRefs.current[i] = el; }} className={cellCls + " text-right"} value={line.disc_percent} placeholder="0"
                          onChange={(e) => setLineField(i, "disc_percent", e.target.value)} onKeyDown={(e) => handleDiscKeyDown(e, i)} />
                      </td>
        ...
        {showCategoryList && (
          <RightSelectPanel
            ...
            entries={[{ sc_id: 0, name: "All Items" }, ...stockCategories].map((sc) => ({
              key: sc.name,
              label: sc.name,
              selected: selectedCategory === sc.name,
              prefix: sc.name === "All Items" ? "◆" : undefined,
            }))}
            onPick={(key) => { setSelectedCategory(key); setShowCategoryList(false); }}

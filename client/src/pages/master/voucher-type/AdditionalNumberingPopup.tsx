...
const colHdr = "py-1.5 px-2 text-[11px] font-bold text-zinc-600 border-r border-zinc-200 last:border-r-0";
    ...
    <div className="flex-1 flex flex-col min-w-0 border-r border-zinc-300">
      ...
      <div className="flex-1">
        {all.map((r, i) => {
          ...
          return (
            <div key={i} className="grid grid-cols-[1fr_80px_1fr_24px] border-b border-zinc-100 items-center">
              <input type="date" className={`${cellCls} ${colCell}`} value={r.applicable_from}
      ...
      <div className="px-3 py-1 text-[11px] text-zinc-400 italic font-sans select-none border-t border-zinc-100">End of List</div>
    </div>
  );
}
    ...
    <div className="flex-1 flex flex-col min-w-0 border-r border-zinc-300 last:border-r-0">
      ...
      <div className="flex-1">
        {all.map((r, i) => {
          const isBlank = i === all.length - 1;
          return (
            <div key={i} className="grid grid-cols-[1fr_1.4fr_24px] border-b border-zinc-100 items-center">
              <input type="date" className={`${cellCls} ${colCell}`} value={r.applicable_from}
                onChange={(e) => setCell(i, { applicable_from: e.target.value })} />

...
import { usePayrollStatutoryDetails } from "./usePayrollStatutoryDetails";
import { PayrollStatutoryDetailsForm } from "./PayrollStatutoryDetailsForm";

export default function PayrollStatutoryDetailsCreate() {
  const navigate = useNavigate();
  const { selectedCompany } = useCompany();
  const companyId = selectedCompany?.company_id;

  const { form, setField, loading, error, setError, success, setSuccess, save } =
    usePayrollStatutoryDetails({ companyId });
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none">
      <PageTitleBar title="Payroll Statutory Details" subtitle={selectedCompany?.name} />
      ...
      <div className="flex-1 flex min-h-0">
        <PayrollStatutoryDetailsForm form={form} setField={setField} firstFieldAutoFocus />

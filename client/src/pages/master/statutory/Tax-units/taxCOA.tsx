...
export default function TaxCOA() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError("Failed to load tax units.");
      } finally {
  ...
  const filteredList = useMemo(() => {
    ...
    if (!q) return taxUnitsList;
    return taxUnitsList.filter(
      (t) =>
        t.name?.toLowerCase().includes(q) ||
        (t.alias && t.alias.toLowerCase().includes(q)) ||
        (t.ecc_number && t.ecc_number.toLowerCase().includes(q)) ||
  ...
  const changeViewItems = [
    ...
    { label: "Tax Units", path: "/master/coa/tax-units" },
    ...
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-800 animate-fade-in font-sans">
      <div className="px-4 py-2 border-b border-zinc-200 bg-zinc-50 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-semibold text-zinc-700 font-sans">Tax Units</span>
        </div>
        <button
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="px-4 py-1.5 border-b border-zinc-100 flex items-center gap-2">
            ...
            <input
              ...
              placeholder="Search by name, alias, or ECC number..."
              autoFocus
            />
            {searchQuery && (
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="p-8 text-center text-xs text-zinc-400 italic">Loading Tax Units...</div>
      ...
      <div className="border-t border-zinc-200 px-4 py-1.5 flex justify-between items-center bg-zinc-50 text-[10px] text-zinc-400 select-none shrink-0">
        <span>{taxUnitsList.length} records</span>

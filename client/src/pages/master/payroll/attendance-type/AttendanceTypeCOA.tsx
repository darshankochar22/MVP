...
export default function AttendanceTypeCOA() {
  ...
  const filtered = useMemo(() => {
    ...
    let result = types;
    if (q) result = result.filter(t => t.name?.toLowerCase().includes(q) || t.alias?.toLowerCase().includes(q) || t.type?.toLowerCase().includes(q));
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.altKey && e.key.toLowerCase() === "c") { e.preventDefault(); navigate("/master/create/attendance-type"); }
    };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-800">
      <div className="px-4 py-2 border-b border-zinc-200 bg-zinc-50 flex items-center justify-between">
        ...
        <button onClick={() => navigate("/master/create/attendance-type")} className="text-[10px] text-zinc-500 hover:text-zinc-800 border border-zinc-200 rounded px-2 py-0.5 bg-white">+ Create</button>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="p-8 text-center text-xs text-zinc-400">Loading attendance types...</div>
            ) : filtered.length === 0 ? (
        ...
        <div className="w-44 border-l border-zinc-200 flex flex-col bg-zinc-50/30 text-[10px]">
          ...
          <button onClick={() => navigate("/master/create/attendance-type")} className="px-3 py-2 text-left hover:bg-zinc-100 border-b border-zinc-100 font-medium text-zinc-600">Alt+C Create</button>

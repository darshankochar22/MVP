...
export default function PayrollUnitCOA() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch { if (!cancelled) setError("Failed to load units."); }
  ...
  const filtered = useMemo(() => {
    ...
    let result = units;
    if (q) result = result.filter(u => u.symbol?.toLowerCase().includes(q) || u.formal_name?.toLowerCase().includes(q) || u.name?.toLowerCase().includes(q));
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.altKey && e.key.toLowerCase() === "c") { e.preventDefault(); navigate("/master/create/payroll-unit"); }
    };
    ...
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-800">
      ...
      <div className="px-4 py-2 border-b border-zinc-200 bg-zinc-50 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => navigate("/master/create/payroll-unit")} className="text-[10px] text-zinc-500 hover:text-zinc-800 border border-zinc-200 rounded px-2 py-0.5 bg-white">+ Create</button>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="px-4 py-1.5 border-b border-zinc-100 flex items-center gap-2">
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery("")} className="text-[10px] text-zinc-400 hover:text-zinc-600">Clear</button>
            )}
          </div>
        ...
        <div className="w-44 border-l border-zinc-200 flex flex-col bg-zinc-50/30 text-[10px]">
          ...
          <button onClick={() => navigate("/master/create/payroll-unit")} className="px-3 py-2 text-left hover:bg-zinc-100 border-b border-zinc-100 font-medium text-zinc-600">Alt+C Create</button>

...
export default function SalesOrderVoucher({ handleAmountConfirm: _ignored, ...props }: Props) {
  return (
    <StockTransferVoucherBody
      {...props}
      config={{ salesPurchaseLabel: "Sales Ledger", hideGodownColumn: true, showActualBilled: true, showOrderNo: true, showPriceLevel: true }}
    />
  );
}

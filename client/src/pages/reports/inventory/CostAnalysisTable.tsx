...
const rate = (v: number | null | undefined, q: number | null | undefined) => {
  const a = Number(v) || 0, b = Number(q) || 0;
  if (!a || !b) return "";
  return new Intl.NumberFormat("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Math.abs(a / b));
};
...
export default function CostAnalysisTable({
  title, companyName, subtitle, periodLabel, rows,
  loading, error, emptyText = "No records found.",
  selectedIndex, onSelectIndex, onActivate, footer,
}: Props) {
  const t = rows.reduce((a, r) => ({
    cQty: a.cQty + r.cost.qty, cVal: a.cVal + r.cost.value,
    rQty: a.rQty + r.revenue.qty, rVal: a.rVal + r.revenue.value,
    bQty: a.bQty + r.balance.qty, bVal: a.bVal + r.balance.value,
    pf: a.pf + r.profit,
        ...
        <table className="w-full border-collapse text-[11px] font-mono select-none">
          <thead className="sticky top-0 bg-[#f4f4f5] border-b border-zinc-300 z-10 text-zinc-700">
            <tr>
              ...
              <th colSpan={3} className="px-2 py-0.5 text-center font-bold border-b border-l border-zinc-200">A. Cost (Expense)</th>
              <th colSpan={3} className="px-2 py-0.5 text-center font-bold border-b border-l border-zinc-200">B. Revenue (Income)</th>
              <th colSpan={3} className="px-2 py-0.5 text-center font-bold border-b border-l border-zinc-200">C. Balance at Cost</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={12} className="px-4 py-8 text-center text-zinc-400 italic">{emptyText}</td></tr>
            ) : rows.map((r, idx) => (
              <tr
                key={r.id}
              ...
              >
                ...
                <td className="px-2 py-1 text-right border-l border-zinc-100">{qty(r.cost.qty, r.unit)}</td>
                <td className="px-2 py-1 text-right">{rate(r.cost.value, r.cost.qty)}</td>
                <td className="px-2 py-1 text-right">{val(r.cost.value)}</td>
                <td className="px-2 py-1 text-right border-l border-zinc-100">{qty(r.revenue.qty, r.unit)}</td>
                ...
                <td className="px-2 py-1 text-right border-l border-zinc-100">{qty(r.balance.qty, r.unit)}</td>
                ...
                <td className="px-2 py-1 text-right border-l border-zinc-100">{val(r.profit)}</td>

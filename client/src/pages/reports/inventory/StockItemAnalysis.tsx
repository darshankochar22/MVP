...
export default function StockItemAnalysis() {
  ...
  React.useEffect(() => {
    ...
    setItemErr(null);
    (window as any).api.report.stockItemAnalysis(companyId, fyId).then((res: any) => {
      if (res.success) setItems(res.items ?? []);
      else setItemErr(res.error || "Failed to load");
  ...
  const loadMonths = React.useCallback((item: ItemRow) => {
  ...
  const loadVouchers = React.useCallback((item: ItemRow) => {
    if (!companyId || !fyId) return;
  ...
  const backToItems  = React.useCallback(() => { setLevel({ step: "items" }); setMonths([]); }, []);
  const backToMonths = React.useCallback((item: ItemRow) => { setLevel({ step: "monthly", item }); setVouchers([]); }, []);
  ...
  React.useEffect(() => {
    ...
    const h = (e: KeyboardEvent) => {
      ...
      else if (e.key === "Enter") { e.preventDefault(); const it = items[itemIdx]; if (it) loadMonths(it); }
      else if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); navigate(-1); }
    };
    window.addEventListener("keydown", h);
    return () => window.removeEventListener("keydown", h);
  }, [level.step, items, itemIdx, loadMonths, navigate]);
  ...
  React.useEffect(() => {
    ...
    const h = (e: KeyboardEvent) => {
      ...
      else if (e.key === "Enter") { e.preventDefault(); loadVouchers(level.item); }
      else if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); backToItems(); }
    };
    window.addEventListener("keydown", h);
    return () => window.removeEventListener("keydown", h);
  ...
  React.useEffect(() => {
    ...
    const h = (e: KeyboardEvent) => {
      ...
      else if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); backToMonths(level.item); }
    };
    window.addEventListener("keydown", h);
    return () => window.removeEventListener("keydown", h);
  ...
  if (level.step === "items") {
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        {reportHeader("Stock Item Analysis", "Movement Analysis — All Items")}
          ...
          <table className="w-full border-collapse text-[11px] font-mono select-none">
            ...
            <tbody>
              {loadingItems ? (
                ...
                <tr><td colSpan={9} className="px-4 py-8 text-center text-zinc-400 italic">No stock items found.</td></tr>
              ) : items.map((it, idx) => (
                <tr
                  key={it.item_id}
                  onClick={() => setItemIdx(idx)}
                  onDoubleClick={() => loadMonths(it)}
  ...
  if (level.step === "monthly") {
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        ...
        <div className="flex items-center gap-4 px-3 py-1 border-t border-zinc-300 bg-zinc-50 text-[10px] font-semibold text-zinc-600 shrink-0">
          <button onClick={backToItems} className="hover:underline hover:text-zinc-900">Q: Back to Items</button>
          <button onClick={() => loadVouchers(it)} className="hover:underline hover:text-zinc-900">Enter: View Vouchers</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
      ...
      <div className="flex items-center gap-4 px-3 py-1 border-t border-zinc-300 bg-zinc-50 text-[10px] font-semibold text-zinc-600 shrink-0">
        <button onClick={() => backToMonths(it)} className="hover:underline hover:text-zinc-900">Q: Back to Monthly</button>

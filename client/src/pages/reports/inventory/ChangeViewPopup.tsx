...
export type RelatedKey =
  | "stockquery" | "movement" | "vouchers" | "costanalysis";

export type ChangeViewSelection =
  | { kind: "view"; key: ViewKey }
  | { kind: "related"; key: RelatedKey };
...
export default function ChangeViewPopup({ currentView, onSelect, onClose }: Props) {
  ...
  const initialIdx = Math.max(0, flat.findIndex(e => e.sel.kind === "view" && e.sel.key === currentView));
  const [idx, setIdx] = React.useState(initialIdx);

  React.useEffect(() => { setIdx(0); }, [search, showMore]);

  React.useEffect(() => {
    const h = (e: KeyboardEvent) => {
      if (e.key === "ArrowDown")      { e.preventDefault(); setIdx(p => Math.min(flat.length - 1, p + 1)); }
      else if (e.key === "ArrowUp")   { e.preventDefault(); setIdx(p => Math.max(0, p - 1)); }
      else if (e.key === "Enter")     { e.preventDefault(); const en = flat[idx]; if (en) onSelect(en.sel); }
      else if (e.key === "Escape")    { e.preventDefault(); onClose(); }
    };
    window.addEventListener("keydown", h, true);
    return () => window.removeEventListener("keydown", h, true);
  }, [flat, idx, onSelect, onClose]);
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/20" onClick={onClose}>
      ...
      >
        <div className="px-3 py-1.5 bg-white border-b-2 border-zinc-900 flex items-center justify-between">
          ...
          <button onClick={() => setShowMore(m => !m)} className="text-[10px] font-semibold text-zinc-600 hover:text-zinc-900 underline">

...
export interface MovRow {
  ...
  name: string;
  unit?: string;
  leftQty: number;  leftValue: number;
  rightQty: number; rightValue: number;
}
...
export default function MovementAnalysisTable({
  title, companyName, subtitle, periodLabel, leftLabel, rightLabel,
  rows, loading, error, emptyText = "No records found.",
  selectedIndex, onSelectIndex, onActivate, footer,
}: Props) {
  const totals = rows.reduce((acc, r) => ({
    lQty: acc.lQty + r.leftQty,  lVal: acc.lVal + r.leftValue,
    rQty: acc.rQty + r.rightQty, rVal: acc.rVal + r.rightValue,
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        ...
        <table className="w-full border-collapse text-[11px] font-mono select-none">
          <thead className="sticky top-0 bg-[#f4f4f5] border-b border-zinc-300 z-10 text-zinc-700">
            <tr>
              ...
              <th colSpan={3} className="px-2 py-0.5 text-center font-bold border-b border-l border-zinc-200">{leftLabel}</th>
              <th colSpan={3} className="px-2 py-0.5 text-center font-bold border-b border-l border-zinc-200">{rightLabel}</th>
            </tr>
            <tr>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="px-4 py-8 text-center text-zinc-400 italic">{emptyText}</td></tr>
            ) : rows.map((r, idx) => (
              <tr
                key={r.id}
              ...
              >
                ...
                <td className="px-2 py-1 text-right border-l border-zinc-100">{fmtQty(r.leftQty, r.unit)}</td>
                <td className="px-2 py-1 text-right">{rate(r.leftValue, r.leftQty)}</td>
                <td className="px-2 py-1 text-right">{fmt(r.leftValue)}</td>
                <td className="px-2 py-1 text-right border-l border-zinc-100">{fmtQty(r.rightQty, r.unit)}</td>
      ...
      <div className="border-t-2 border-zinc-300 bg-[#f4f4f5] px-3 py-1.5 flex font-mono text-[11px] font-bold text-zinc-900 shrink-0">
        ...
        <span className="w-24 text-right border-l border-zinc-300 pr-1">{fmtQty(totals.lQty)}</span>
        ...
        <span className="w-24 text-right border-l border-zinc-300 pr-1">{fmtQty(totals.rQty)}</span>

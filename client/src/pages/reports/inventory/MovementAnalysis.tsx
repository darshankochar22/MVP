...
const fmtVal = (v: number) =>
  v === 0 ? "" : new Intl.NumberFormat("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(v);
const fmtQty = (v: number, unit?: string) => {
  if (v === 0) return "";
  const s = Math.abs(v).toLocaleString("en-IN", { minimumFractionDigits: 0, maximumFractionDigits: 3 });
...
export default function MovementAnalysis() {
  ...
  const [movData,    setMovData]    = React.useState<{ inward: PartyTotal[]; outward: PartyTotal[] } | null>(null);
  ...
  const openMovement = React.useCallback((item: StockItem) => {
    ...
    (window as any).api.report
      ...
      .catch((e: any) => { setMovErr(e.message); setMovLoading(false); });
  ...
  React.useEffect(() => {
    ...
    const h = (e: KeyboardEvent) => {
      ...
      else if (e.key === "Enter")     { e.preventDefault(); const it = filtered[selectIdx]; if (it) openMovement(it); }
  ...
  React.useEffect(() => {
    ...
    const h = (e: KeyboardEvent) => {
      if (e.key === "ArrowDown") { e.preventDefault(); setDetailIdx(p => Math.min(level.filteredRows.length - 1, p + 1)); }
      else if (e.key === "ArrowUp") { e.preventDefault(); setDetailIdx(p => Math.max(0, p - 1)); }
      else if (e.key === "Enter") {
        e.preventDefault();
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        ...
        <SelectionPopup
          ...
          onAccept={(i) => { const it = filtered[i]; if (it) openMovement(it); }}
          onCancel={() => navigate(-1)}
      ...
      <ItemVoucherAnalysis
        ...
        footer={
          <div className="flex items-center gap-6 px-3 py-1 border-t border-zinc-300 bg-white text-[10px] font-semibold text-zinc-600 shrink-0">
            <button
              onClick={() => setLevel({ step: "movement", item: level.item, allRows: allRowsRef.current })}
  ...
  const inTotal  = { qty: inRows.reduce((s, r) => s + r.qty, 0),  value: inRows.reduce((s, r) => s + r.value, 0) };
  const outTotal = { qty: outRows.reduce((s, r) => s + r.qty, 0), value: outRows.reduce((s, r) => s + r.value, 0) };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
          ...
          <table className="w-full border-collapse text-[11px] font-mono">
            ...
            <tbody>
              ...
              <tr className="bg-zinc-50 border-t border-zinc-300">
                <td colSpan={3} className="px-3 py-1 font-bold text-zinc-900 tracking-wide">Movement Inward:</td>
              </tr>
              <tr className="bg-zinc-50 border-b border-zinc-200">
                <td colSpan={3} className="px-6 py-0.5 text-zinc-500 italic text-[10px]">Suppliers</td>
              </tr>
              {inRows.length === 0 ? (
                <tr><td colSpan={3} className="px-6 py-2 text-zinc-400 italic">No inward movement</td></tr>
              ) : inRows.map((r, idx) => (
                <tr
                  key={r.name}
              ...
              <tr className="bg-zinc-50 border-t border-zinc-300">
                <td colSpan={3} className="px-3 py-1 font-bold text-zinc-900 tracking-wide">Movement Outward:</td>
              </tr>
              <tr className="bg-zinc-50 border-b border-zinc-200">
                <td colSpan={3} className="px-6 py-0.5 text-zinc-500 italic text-[10px]">Buyers</td>
              </tr>
              {outRows.length === 0 ? (
                <tr><td colSpan={3} className="px-6 py-2 text-zinc-400 italic">No outward movement</td></tr>
              ) : outRows.map((r, idx) => (
                <tr
                  key={r.name}
      ...
      <div className="flex items-center gap-6 px-3 py-1 border-t border-zinc-300 bg-white text-[10px] font-semibold text-zinc-600 shrink-0">
        <button
          onClick={() => { setLevel({ step: "select" }); setMovData(null); }}
          className="hover:text-zinc-900"
        >
          Q: Quit
        </button>

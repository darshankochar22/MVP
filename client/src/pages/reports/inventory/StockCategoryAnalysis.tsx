...
interface CategoryRef { category_id: number; category_name: string; }
...
export default function StockCategoryAnalysis() {
  ...
  React.useEffect(() => {
    if (!companyId) { setCatListLoading(false); return; }
  ...
  const filtered = React.useMemo(() =>
    search.trim() === "" ? catList : catList.filter(c => c.category_name.toLowerCase().includes(search.toLowerCase())),
    [catList, search]
  );
  ...
  const loadItems = React.useCallback((category: CategoryRef) => {
  ...
  const loadVouchers = React.useCallback((category: CategoryRef, item: ItemRow) => {
    if (!companyId || !fyId) return;
  ...
  React.useEffect(() => {
    const h = (e: KeyboardEvent) => {
      if (level.step === "select") {
        ...
        else if (e.key === "Enter") { e.preventDefault(); const c = filtered[selectIdx]; if (c) loadItems(c); }
        ...
        else if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); loadItems(level.category); }
      }
    };
    window.addEventListener("keydown", h);
    return () => window.removeEventListener("keydown", h);
  }, [level, filtered, selectIdx, items, itemIdx, vouchers, voucherIdx, loadItems, loadVouchers, backToSelect, navigate]);
  ...
  if (level.step === "select") {
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        ...
        <SelectionPopup
          title="Select Stock Category" fieldLabel="Name of Stock Category" listLabel="List of Stock Categories"
          companyName={selectedCompany?.name}
          items={filtered.map(c => ({ id: c.category_id, name: c.category_name }))}
          index={selectIdx} loading={catListLoading} search={search}
          onSearchChange={setSearch} onIndexChange={setSelectIdx}
          onAccept={(i) => { const c = filtered[i]; if (c) loadItems(c); }}
          onCancel={() => navigate(-1)}
  ...
  if (level.step === "items") {
    ...
    const subtitle = c.category_id === PRIMARY_ID ? "Primary — All Items" : `Category: ${c.category_name}`;
    ...
    return (
      <MovementAnalysisTable
        title="Stock Category Movement Analysis" companyName={selectedCompany?.name} subtitle={subtitle}
  ...
  return (
    <ItemVoucherAnalysis
      ...
      footer={<FooterBar><button onClick={() => loadItems(c)} className="hover:underline hover:text-zinc-900">Q: Back to Items</button><span className="text-zinc-400">Enter: Open voucher</span></FooterBar>}
    />
  );
}

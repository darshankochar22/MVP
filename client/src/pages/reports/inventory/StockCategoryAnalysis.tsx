...
export default function StockCategoryAnalysis() {
  ...
  const [categoryErr, setCategoryErr] = React.useState<string | null>(null);
  const [categoryIdx, setCategoryIdx] = React.useState(0);

  React.useEffect(() => {
    if (!companyId || !fyId) { setLoadingCategories(false); return; }
    setLoadingCategories(true);
    setCategoryErr(null);
    (window as any).api.report.stockCategoryAnalysis(companyId, fyId).then((res: any) => {
      if (res.success) setCategories(res.categories ?? []);
      else setCategoryErr(res.error || "Failed to load");
  ...
  const backToCategories = React.useCallback(() => { setLevel({ step: "categories" }); setItems([]); }, []);
  const backToItems      = React.useCallback((category: CategoryRow) => { setLevel({ step: "items", category }); setMonths([]); }, []);
  const backToMonths     = React.useCallback((category: CategoryRow, item: ItemRow) => { setLevel({ step: "monthly", category, item }); setVouchers([]); }, []);

  // Keyboard nav — Level 1
  React.useEffect(() => {
    if (level.step !== "categories") return;
    const h = (e: KeyboardEvent) => {
      if (e.key === "ArrowDown") { e.preventDefault(); setCategoryIdx(p => Math.min(categories.length - 1, p + 1)); }
      else if (e.key === "ArrowUp") { e.preventDefault(); setCategoryIdx(p => Math.max(0, p - 1)); }
      else if (e.key === "Enter") { e.preventDefault(); const c = categories[categoryIdx]; if (c) loadItems(c); }
      else if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); navigate(-1); }
    };
    window.addEventListener("keydown", h);
    return () => window.removeEventListener("keydown", h);
  }, [level.step, categories, categoryIdx, loadItems, navigate]);
  ...
  React.useEffect(() => {
    ...
    const h = (e: KeyboardEvent) => {
      ...
      else if (e.key === "Enter") { e.preventDefault(); const it = items[itemIdx]; if (it) loadMonths(level.category, it); }
      else if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); backToCategories(); }
    };
    window.addEventListener("keydown", h);
    return () => window.removeEventListener("keydown", h);
  }, [level, items, itemIdx, loadMonths, backToCategories]);
  ...
  React.useEffect(() => {
    ...
    const h = (e: KeyboardEvent) => {
      ...
      else if (e.key === "Enter") { e.preventDefault(); loadVouchers(level.category, level.item); }
  ...
  React.useEffect(() => {
    ...
    const h = (e: KeyboardEvent) => {
      ...
      else if (e.key === "Escape" || e.key === "Backspace") { e.preventDefault(); backToMonths(level.category, level.item); }
    };
    window.addEventListener("keydown", h);
    return () => window.removeEventListener("keydown", h);
  ...
  // ── Level 1: All Stock Categories ────────────────────────────────────────
  if (level.step === "categories") {
    const totals = categories.reduce((acc, c) => ({
      oQty: acc.oQty + c.opening_qty, oVal: acc.oVal + c.opening_value,
      iQty: acc.iQty + c.in_qty,      iVal: acc.iVal + c.in_value,
      outQty: acc.outQty + c.out_qty, outVal: acc.outVal + c.out_value,
      cQty: acc.cQty + c.closing_qty, cVal: acc.cVal + c.closing_value,
    }), { oQty: 0, oVal: 0, iQty: 0, iVal: 0, outQty: 0, outVal: 0, cQty: 0, cVal: 0 });

    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        {reportHeader("Stock Category Analysis", "Movement Analysis — All Categories")}
          ...
          <table className="w-full border-collapse text-[11px] font-mono select-none">
            ...
            <tbody>
              {loadingCategories ? (
                ...
                <tr><td colSpan={9} className="px-4 py-8 text-center text-zinc-600">{categoryErr}</td></tr>
              ) : categories.length === 0 ? (
                <tr><td colSpan={9} className="px-4 py-8 text-center text-zinc-400 italic">No stock categories found.</td></tr>
              ) : categories.map((c, idx) => (
                <tr
                  key={c.category_id}
                  onClick={() => setCategoryIdx(idx)}
                  onDoubleClick={() => loadItems(c)}
                  className={`border-b border-zinc-100 cursor-pointer ${idx === categoryIdx ? "bg-[#e4e4e7] text-zinc-950 font-bold" : "hover:bg-zinc-50 text-zinc-800"}`}
                >
                  <td className="px-3 py-1">{c.category_name}</td>
                  <td className="px-2 py-1 text-right border-l border-zinc-100">{fmtQty(c.opening_qty)}</td>
                  <td className="px-2 py-1 text-right">{fmt(c.opening_value)}</td>
                  <td className="px-2 py-1 text-right border-l border-zinc-100">{fmtQty(c.in_qty)}</td>
                  <td className="px-2 py-1 text-right">{fmt(c.in_value)}</td>
                  <td className="px-2 py-1 text-right border-l border-zinc-100">{fmtQty(c.out_qty)}</td>
                  <td className="px-2 py-1 text-right">{fmt(c.out_value)}</td>
                  <td className="px-2 py-1 text-right border-l border-zinc-100">{fmtQty(c.closing_qty)}</td>
                  <td className="px-2 py-1 text-right">{fmt(c.closing_value)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex items-center gap-4 px-3 py-1 border-t border-zinc-300 bg-zinc-50 text-[10px] font-semibold text-zinc-600 shrink-0">
          ...
          <span className="text-zinc-400">Enter: Drill into category</span>
        </div>
      </div>
    );
  }
  ...
  if (level.step === "items") {
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
        {reportHeader("Stock Category Analysis", `Category: ${c.category_name}`)}
        <div className="flex-1 overflow-y-auto">
          <table className="w-full border-collapse text-[11px] font-mono select-none">
            <MovHeader />
            <tbody>
              {loadingItems ? (
                ...
                <tr><td colSpan={9} className="px-4 py-8 text-center text-zinc-400 italic">No items in this category.</td></tr>
              ) : items.map((it, idx) => (
                <tr
                  key={it.item_id}
                  onClick={() => setItemIdx(idx)}
                  onDoubleClick={() => loadMonths(c, it)}
        ...
        <MovTotalRow label="Grand Total" oQty={totals.oQty} oVal={totals.oVal} iQty={totals.iQty} iVal={totals.iVal} outQty={totals.outQty} outVal={totals.outVal} cQty={totals.cQty} cVal={totals.cVal} />
        <div className="flex items-center gap-4 px-3 py-1 border-t border-zinc-300 bg-zinc-50 text-[10px] font-semibold text-zinc-600 shrink-0">
          <button onClick={backToCategories} className="hover:underline hover:text-zinc-900">Q: Back to Categories</button>
  ...
  if (level.step === "monthly") {
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
            ...
            <tbody>
              {loadingMonths ? (
                ...
                <tr
                  ...
                  onDoubleClick={() => loadVouchers(c, it)}
        ...
        <div className="flex items-center gap-4 px-3 py-1 border-t border-zinc-300 bg-zinc-50 text-[10px] font-semibold text-zinc-600 shrink-0">
          <button onClick={() => backToItems(c)} className="hover:underline hover:text-zinc-900">Q: Back to Items</button>
          <button onClick={() => loadVouchers(c, it)} className="hover:underline hover:text-zinc-900">Enter: View Vouchers</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
      ...
      <div className="flex items-center gap-4 px-3 py-1 border-t border-zinc-300 bg-zinc-50 text-[10px] font-semibold text-zinc-600 shrink-0">
        <button onClick={() => backToMonths(c, it)} className="hover:underline hover:text-zinc-900">Q: Back to Monthly</button>

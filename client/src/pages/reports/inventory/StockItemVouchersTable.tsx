    ...
    <div className="flex-1 flex flex-col h-full bg-white select-none text-zinc-900 font-sans text-[11px]">
      ...
      <div className="flex-1 overflow-y-auto">
        <table className="w-full border-collapse text-[11px] font-mono select-none">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={10} className="px-4 py-8 text-center text-zinc-400 italic">Loading...</td></tr>
            ) : error ? (
              <tr><td colSpan={10} className="px-4 py-8 text-center text-zinc-600">{error}</td></tr>
            ) : rows.length === 0 ? (
              ...
              >
                ...
                <td className={`px-2 py-1 truncate max-w-xs ${!row.voucher_id ? "italic" : ""}`}>{row.particulars}</td>
                <td className="px-2 py-1">{row.voucher_type}</td>
                <td className="px-2 py-1 text-right">{row.voucher_number || ""}</td>
                <td className={`${num} border-l border-zinc-100`}>{fmtQty(row.inwards_qty, unit)}</td>
                <td className={num}>{fmt(row.inwards_value)}</td>
                <td className={`${num} border-l border-zinc-100`}>{fmtQty(row.outwards_qty, unit)}</td>
                <td className={num}>{fmt(row.outwards_value)}</td>
                <td className={`${num} border-l border-zinc-100`}>{fmtQty(row.closing_qty, unit)}</td>
      ...
      <div className="border-t-2 border-zinc-300 bg-[#f4f4f5] px-3 py-1.5 flex font-mono text-[11px] font-bold text-zinc-900 shrink-0">
        ...
        <span className="w-28 shrink-0" /><span className="w-16 shrink-0" />
        ...
        <span className="w-24 text-right pr-1 border-l border-zinc-300">{fmtQty(t.outQ, unit)}</span>
        <span className="w-28 text-right pr-1">{fmt(t.outV)}</span>
        <span className="w-24 text-right pr-1 border-l border-zinc-300">{fmtQty(finalCQty, unit)}</span>
